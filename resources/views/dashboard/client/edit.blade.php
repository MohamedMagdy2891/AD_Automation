...
@push('title') تعديل بيانات العميل {{ $row->full_name }}@endpush
@push('header') تعديل بيانات العميل {{ $row->full_name }}@endpush
@section('content')
    ...
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="card">
            ...
            <div class="card-header bg-info">
                <div class="card-title text-center text-light pb-1">تعديل بيانات العميل {{ $row->full_name }}</div>
            </div>
            <form class="card-body" action="{{ URL::route('dashboard.client.update',$row->id) }}" method="POST">
                @csrf
                @method('PUT')
